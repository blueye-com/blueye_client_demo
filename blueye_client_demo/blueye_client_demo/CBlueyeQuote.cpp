	...
	else
	{
		...
		else if(_blueye_data->type_ == REQ_TYPE_QUOTE_KLINE_DATA || _blueye_data->type_ == REQ_TYPE_QUOTE_BATCH_KLINE_DATA)
		{
			if (_blueye_data->quote_head_.t1 == ROM_QUOTE_HEAD::P1M)
			{
				...
				//将ROM_TINY_KLINE_DATA 转化为 ROM_KLINE_DATA 以作后续处理
				...
				char _str_symbol[SYMBOL_LEN + 1] = { 0 };
				memcpy(_str_symbol, (char*)_blueye_data->quote_head_.symbol, SYMBOL_LEN);
				ManyMinutesDataConversion(_rec_count, _blueye_data->quote_head_.exchange, _str_symbol,
					_kline, _translated_kline, KLINE_TYPE_MINUTE5_DATA);
			}
			else
			{
				ipfs.write_box_data_daily(_blueye_data, _rec_count);
				for (int i = 0; i < _rec_count; i++)
				{
				...
				std::cout << "total REQ_TYPE_QUOTE_DAILY_DATA records count is " << _rec_count << std::endl;
			}
		}
